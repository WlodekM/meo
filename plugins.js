...
const wlp_version = 'builtin'

logCategoryStyled("main", "DarkGoldenRod", "font-size: 1.5em",
	`WL-plugins for meo version ${wlp_version}`)
...
const wl = window.wl = {
	util: {
		...
		/**
		 * apply mixin(s) to a function
		 * @param {Function} fn the function to work on
		 * @param {string[]} mixins mixins to apply
		 * @param {string[]} argnames the functions arguments
		...
		mixinStr(fn, mixins, argnames) {
			...
			// only really tracks appends
			const patches = fn.patches ?? [];
			...
			for (const append of appends) {
				...
				for (const patch of patches) {
					if (patch[0] > append.line) continue;
					offset += patch[1]
				}
				// console.log(append, offset)
				source.splice(+append.line + offset, 0, append.code)
				patches.push([append.line, append.code.split('\n').length]);
			}
...
async function loadWlPluginsPlugins(exec = true) {
	// 'use strict';

	wl.util.updateStatus("Fetching plugin list...")
	logCategory("plugins", "blue", "Loading plugin list")
	let presp = await fetch(localStorage.hasOwnProperty("test") ?
		"http://localhost:5056/plugins.json" :
		"https://wlodekm.github.io/wl-plugins/plugins.json", { cache: "no-store" }
	)
